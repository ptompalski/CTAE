...
`vol()` is a convenience wrapper for estimating total and merchantable tree volume using the volume models implemented in `CanadaForestAllometry`. Rather than requiring users to select a specific model, `vol()` consults an internal model registry and automatically determines which models are applicable for each tree based on:
...
The jurisdiction-specific merchantability criteria used by `CanadaForestAllometry` can be inspected in the internal dataset `merchcrit`, which documents the values applied for each jurisdiction (and, where applicable, by species or subregion).
