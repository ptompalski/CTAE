...
GI^{b_2(A)}\$\$ where `A` is breast-height age and `GI` is the growth
intercept (cm/year). Following the source, parameter \\b_1\\ was fitted
on the log scale and enters the model as \\e^{b_1}\\ (Ratkowsky
